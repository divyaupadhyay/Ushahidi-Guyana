		...
		<div class="rb_list-and-map-box">
			...
			<?php
				...
				//$form_data = ORM::Factory('form_response')->join('form_response', 'form_field_id', 'incident_id')->where('incident_id', $incident_id)->find_all();
					...
					//$depts = ORM::factory('form_response')->find_all()->as_array('i_id','dept_name')
	$field = ORM::factory('form_field')->select_list('id', 'field_name');
	$data = ORM::Factory('form_response')->where('incident_id', $incident_id)->select_list('id', 'form_response')->find_all();
							
	//$data = ORM::factory('form_response')->select_list('incident_id', 'form_response');
/**foreach ($data as $key => $value) {
	
	echo $value;
	echo "<img src="+$incident_thumb+">";
}**/
				...
				<div id="incident_<?php echo $incident_id ?>" class="rb_report <?php echo $incident_verified_class; ?>">
					...
					<div class="r_details">
						...
						<div class="r_description"> 
						  ...
						  <?php $map = array_map(NULL, $field, $data);
